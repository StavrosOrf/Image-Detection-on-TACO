...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 plastic.py train --dataset=/path/to/plastic/dataset --weights=coco
    # Resume training a model that you had trained earlier
    python3 plastic.py train --dataset=/path/to/plastic/dataset --weights=last
    # Train a new model starting from ImageNet weights
    python3 plastic.py train --dataset=/path/to/plastic/dataset --weights=imagenet
    # Apply color splash to an image
    python3 plastic.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>
    # Apply color splash to video using the last weights you trained
    python3 plastic.py splash --weights=last --video=<URL or path to file>
"""

#  python setup.py clean --all install
#  python taco.py train --dataset=./dataset --weights=123
...
ROOT_DIR_1="../../"
DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR_1, "logs")
...
# 10 classes
# classes = ['Plastic bag & wrapper','Paper','Unlabeled litter','Bottle','Bottle cap','Can','Other plastic','Carton','Cup','Straw'] 
...
class tacoDataset(utils.Dataset):

    def load_taco(self, dataset_dir, subset):
        """Load a subset of the plastic dataset.
        ...
        for a in annotations:
            ...
            # name_dict = {"plastic": 1,"metal": 2,"paper": 3}
            # key = tuple(name_dict)
            ...
            self.add_image("object", # for a single class just add the name here
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        """
        # If not a plastic dataset image, delegate to parent class.
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = tacoDataset()
    dataset_train.load_taco(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = tacoDataset()
    dataset_val.load_taco(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect plastics.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/plastic/dataset/",
                        help='Directory of the plastic dataset')
    ...
    if args.command == "train":
        ...
        class InferenceConfig(tacoConfig):
    ...
    if args.weights.lower() == "coco":
    ...
    elif args.weights.lower() == "trashnet":
        weights_path = COCO_WEIGHTS_PATH
        # Download weights file
        if not os.path.exists(weights_path):
            utils.download_trained_weights(weights_path)
        ...
        weights_path = "best_model_resnet50_v6.h5"
