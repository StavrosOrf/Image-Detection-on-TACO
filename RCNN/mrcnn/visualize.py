...
def display_instances_10fps(image, boxes, masks, class_ids, class_names,scores, colors, real_time=True):    
    # Number of instances
    N = boxes.shape[0]
    #white = (255, 255, 255)
    #if not N:
    ...
    for i in range(N):
        ...
        #Center of bounding boxes 
        # print (boxes[i],label)
        # print (masked_image.shape[0],masked_image.shape[1])
        center = ((x1+x2)/2,(y1+y2)/2)
        if ((center[1]>600) and (center[1]<670)):
            print ("Item type: "+label+" with center coordinates: "+str(center)+" confidence score: " + str(score))
